...
func (l *Logger) newZap() (err error) {
	...
	if config.C == nil {
		config.InitConfig("")
		config.C.SetDefault("system.debug", "true")
	}
	if config.C.GetBool("system.debug") {
...
func (l *Logger) updateLogFile() {
	config.C.SetDefault("system.saveDays", "7")
	saveDays := config.C.GetFloat64("system.saveDays")
	for {
		now := time.Now()
		// 计算下一个零点
